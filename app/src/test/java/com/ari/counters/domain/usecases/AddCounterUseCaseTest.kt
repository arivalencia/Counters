...
class AddCounterUseCaseTest {
    ...
    private lateinit var addCounterUseCase: AddCounterUseCase
    ...
    fun onBefore() {
        ...
        addCounterUseCase = AddCounterUseCase(counterRepository)
    }

    @Test
    fun `when the api return an error then get an error`() = runBlocking {
        // Given
        coEvery { counterRepository.addCounter(any()) } returns Response.Error("This is an error")

        // When
        val result = addCounterUseCase("any")
    ...
    fun `when the api return a list then get and save data in local data base`() = runBlocking {
        ...
        coEvery { counterRepository.addCounter(any()) } returns Response.Success(list)
