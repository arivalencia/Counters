...
class DecrementCounterUseCaseTest {
    ...
    private lateinit var decrementCounterUseCase: DecrementCounterUseCase
    ...
    fun `when the api return an error then get an error`() = runBlocking {
        // Given
        coEvery { counterRepository.decrementCounter(any()) } returns Response.Error("This is an error")

        // When
        val result = decrementCounterUseCase("")
    ...
    fun `when the api return a list then get and save data in local data base`() = runBlocking {
        ...
        coEvery { counterRepository.decrementCounter(any()) } returns Response.Success(list)

        // When
        val result: Result<CounterDomain> = decrementCounterUseCase("dkcjbsd")
